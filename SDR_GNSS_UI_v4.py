...
#        self.base_path = "C:/Users/sting/Downloads/PocketSDR-master/PocketSDR-master/bin"
        ...
        try:
            logo_path = os.path.join(self.base_path, "SRT_Logo_2.png")
...
#        columns = ("CH", "RF", "SAT", "SIG", "PRN", "LOCK(s)", "C/N0", "COFF(ms)", "DOP(Hz)", "ADR(cyc)", "SYNC", "NAV", "ERR", "LOL", "FEC")
        ...
        self.canvas = FigureCanvasTkAgg(self.fig1, master=self.kinematic_tab)
        self.canvas.get_tk_widget().pack(fill="both", expand=True)
        ...
        self.canvas = FigureCanvasTkAgg(self.fig1, master=root)
        self.canvas.draw()
    ...
    def update_kinematic(self, velocity, acceleration, jerk, time):
        ...
        self.ax1[0].set_xlabel('Time (s)', fontsize=10)
        self.ax1[0].set_ylabel('Velocity (m/s)', fontsize=10)
        self.ax1[1].set_xlabel('Time (s)', fontsize=10)
        self.ax1[1].set_ylabel('Acceleration (m/s^2)', fontsize=10)
        self.ax1[2].set_xlabel('Time (s)', fontsize=10)
        self.ax1[2].set_ylabel('Jerk (m/s^3)', fontsize=10)
