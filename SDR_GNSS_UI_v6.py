...
import subprocess, threading, re, shlex, os, time, socket, signal, sys, traceback, random
...
#        self.base_path = "C:/Users/sting/Downloads/PocketSDR-master/PocketSDR-master/bin"
        ...
        try:
            ...
            logo_image = Image.open(logo_path).resize((120, 100), Image.Resampling.LANCZOS)
...
#        columns = ("CH", "RF", "SAT", "SIG", "PRN", "LOCK(s)", "C/N0", "COFF(ms)", "DOP(Hz)", "ADR(cyc)", "SYNC", "NAV", "ERR", "LOL", "FEC")
        ...
        labels = ["Time", "Latitude", "Longitude", "Altitude", "Fix", "Buffer", "Search", "Lock", "Velocity", "Acceleration", "Jerk"]
        self.status_labels = {}
        ...
        VELOCITY_LINE_COLOR = '#87CEEB' # SkyBlue
        ACCELERATION_LINE_COLOR = '#98FB98' # PaleGreen
        JERK_LINE_COLOR = '#FFD700'     # Gold
        ...
        self.ax1[0].set_ylabel('Velocity (m/s)', fontsize=8)
        #self.ax1[1].set_xlabel('Time (s)', fontsize=8)
        self.ax1[1].set_ylabel('Acceleration (m/s^2)', fontsize=8)
        self.ax1[2].set_xlabel('Time (s)', fontsize=8)
        self.ax1[2].set_ylabel('Jerk (m/s^3)', fontsize=8)
...
#        cmd = shlex.split(f"./pocket_trk -sig L1CA -prn 1-32 -sig G1CA -prn -7-6/1-27 -sig E1B -prn 1-36 -sig E5AI -prn 1-36 -sig E5BI -prn 1-36 -sig B1I -prn 1-63 -sig B1CD -prn 1-63 -sig B2AD -prn 1-63 -sig B2I -prn 1-63 -sig B2BI -prn 1-63 -sig B3I -prn 1-63 -nmea :{self.tcpport}")
            ...
            try:
                ...
                while True:
                    ...
                    while '\n' in buffer:
                        ...
                        if lineNMEA and self.nmeaStatus == 0:
    ...
    def process_line(self, line_to_process):
        ...
        if "GGA" in line_to_process:
            ...
            if time_from_gga is not None:
                ...
                print(f"GNGGA processed. Current UTC Seconds: {timing:.3f}") # Keep for debugging
            else:
                # print(f"GNGGA line received but failed to parse time: {line_to_process}") # Keep for debugging
                pass # Suppress repeated messages if it's just normal, uninteresting GGA
            return # GGA primarily provides time, not velocity for calculation

        # Process RMC or VTG for velocity, acceleration, jerk
        # Look for "RMC" or "VTG" anywhere in the line, as talker ID can vary
        self.velocity = None
        if "RMC" in line_to_process and (self.state == 0 or self.state == 2):
            self.state = 2
        ...
        elif "VTG" in line_to_process and (self.state == 0 or self.state == 1):
            self.state = 1
        ...
        if self.velocity is not None and self.current_utc_seconds is not None:
            ...
            if self.last_processed_time is not None:
                ...
                if delta_time > 0.001: # Ensure a meaningful time difference
