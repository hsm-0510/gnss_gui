...
import subprocess, threading, re, shlex, os, time, socket, traceback, random
...
#        self.base_path = "C:/Users/sting/Downloads/PocketSDR-master/PocketSDR-master/bin"
        ...
        labelAll = ["Time", "Latitude", "Longitude", "Altitude"]
        self.state_labels = {}
    ...
    def parse_position_status(self, line):
        ...
        if len(parts) >= 12:
            if parts[2] != '0.00000000' and parts[1] != self.old_time:
                ...
                self.status_labels["Latitude"].config(text=f"Latitude: {parts[2]}", foreground="#6FAE2B")
                self.status_labels["Longitude"].config(text=f"Longitude: {parts[3]}", foreground="#6FAE2B")
                self.status_labels["Altitude"].config(text=f"Altitude: {parts[4]}", foreground="#6FAE2B")
                self.state_labels["Time"].config(text=f"Time: {parts[0]} {parts[1]}")
                self.state_labels["Latitude"].config(text=f"Latitude: {parts[2]}", foreground="#6FAE2B")
                self.state_labels["Longitude"].config(text=f"Longitude: {parts[3]}", foreground="#6FAE2B")
                self.state_labels["Altitude"].config(text=f"Altitude: {parts[4]}", foreground="#6FAE2B")
                ...
                self.state_labels["Time"].config(text=f"Time: {parts[0]} {parts[1]}")
                self.state_labels["Latitude"].config(text=f"Latitude: {parts[2]}")
                self.state_labels["Longitude"].config(text=f"Longitude: {parts[3]}")
                self.state_labels["Altitude"].config(text=f"Altitude: {parts[4]}")
                self.status_labels["Fix"].config(text=f"Fix: {parts[5]}")
                self.status_labels["Buffer"].config(text=f"Buffer: {parts[8]}")
                self.status_labels["Search"].config(text=f"Search: {parts[10]}")
                self.status_labels["Lock"].config(text=f"Lock: {parts[12]} {parts[13]}")
