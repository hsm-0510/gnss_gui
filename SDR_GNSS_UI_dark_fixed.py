...
#        self.base_path = "C:/Users/sting/Downloads/PocketSDR-master/PocketSDR-master/bin"
        ...
        style.configure("TButton", font=("Helvetica", 10, "bold"), background="#2c3e50", foreground="white")
        style.configure("Treeview", font=("Helvetica", 10), background="#1e272e", foreground="white", fieldbackground="#1e272e")
        style.configure("Treeview.Heading", font=("Helvetica", 12, "bold"), background="#2c3e50", foreground="white")
        style.configure("TLabelframe.Label", font=("Helvetica", 16, "bold"), background="#d9d9d9", foreground="black")
        ...
        try:
            ...
            logo_image = Image.open(logo_path).resize((350, 100), Image.Resampling.LANCZOS)
            self.logo_photo = ImageTk.PhotoImage(logo_image)
            self.logo_label = ttk.Label(self.top_frame, image=self.logo_photo, background="#151b23")
            self.logo_label.grid(row=0, column=0, padx=50, pady=10)
        except Exception as e:
            print(f"Logo loading failed: {e}")
        
        self.heading = tk.Label(self.top_frame, text="  SDR-BASED GNSS RECEIVER", font=("Helvetica", 50, "bold"), bg="#151b23", fg="white", anchor="w")
        ...
        # Command frame
        self.cmd_frame = ttk.LabelFrame(self.main_frame, text="Controls", padding="10", style="TLabelframe")
        ...
        # Status frame
        self.status_frame = ttk.LabelFrame(self.data_frame, text="Position & Status", padding="10", width=500, height=300, style="TLabelframe")
        self.status_frame.grid(row=0, column=1, sticky="nsw", padx=5, pady=5)
        self.status_frame.grid_propagate(False)
        
        labels = ["Time", "Latitude", "Longitude", "Altitude", "Fix", "Buffer", "Search", "Lock", "Velocity"]
        self.status_labels = {}
        for i, label in enumerate(labels):
            self.status_labels[label] = tk.Label(self.status_frame, text=f"{label}: ", font=("Helvetica", 9, "bold"), width=67,bg="#3a444d", fg="white", anchor="w")
            self.status_labels[label].grid(row=i, column=0, sticky="we", pady=1)

        # Satellite table
        self.tree_frame = ttk.LabelFrame(self.data_frame, text="Satellite Data", padding="10", style="TLabelframe")
        self.tree_frame.grid(row=0, column=0, sticky="nsew", padx=0, pady=5)
...
#        columns = ("CH", "RF", "SAT", "SIG", "PRN", "LOCK(s)", "C/N0", "COFF(ms)", "DOP(Hz)", "ADR(cyc)", "SYNC", "NAV", "ERR", "LOL", "FEC")
        ...
        self.statusF = ttk.Treeview(self.status_frame)
        ...
        self.statusF.grid(row=9, column=1, sticky="ns")
        
        self.tree.tag_configure("oddrow", background="#2f3640")
        self.tree.tag_configure("evenrow", background="#353b48")
        
        scrollbar = ttk.Scrollbar(self.tree_frame, orient="vertical", command=self.tree.yview)
        self.tree.configure(yscrollcommand=scrollbar.set)
        scrollbar.grid(row=0, column=1, sticky="ns")
        
        scrollbar_sf = ttk.Scrollbar(self.status_frame, orient="vertical", comman=self.statusF.yview)
        self.statusF.configure(yscrollcommand=scrollbar_sf.set)
        scrollbar_sf.grid(row=0, column=1, sticky="ns")
        ...
        self.statusF.bind("<Configure>", self.on_treeview_resize)

        # Plot frame
        self.plot_frame = ttk.LabelFrame(self.main_frame, text="C/N0 Plot", padding="10", style="TLabelframe")
        self.plot_frame.grid(row=2, column=0, columnspan=2, sticky="nsew", pady=10)
        
        self.fig, self.ax = plt.subplots(figsize=(6, 4))
        self.fig.subplots_adjust(top=0.9, bottom=0.15)
        # Set dark background for figure and axes at initialization
        self.fig.patch.set_facecolor('#151b23')       # Figure background
        self.ax.set_facecolor('#1e272e')              # Axes background

        # Set axis text and spine colors
        ...
        self.ax.set_title("Carrier-to-Noise Ratio per Satellite", fontsize=14, color="white")
        self.ax.set_xlabel("Satellite", fontsize=9, color="white")  # Set x-axis label
        self.ax.set_ylabel("C/N0 (dB-Hz)", fontsize=9, color="white")  # Set y-axis label
...
#                 self.status_labels["Altitude"].config(text=f"Altitude: {parts[4]}", foreground="black", background="#00ff00")
                self.status_labels["Latitude"].config(text=f"Latitude: {parts[2]}", foreground="#00ff00")
                self.status_labels["Longitude"].config(text=f"Longitude: {parts[3]}", foreground="#00ff00")
                self.status_labels["Altitude"].config(text=f"Altitude: {parts[4]}", foreground="#00ff00")
                self.status_labels["Fix"].config(text=f"Fix: {parts[5]}")
                self.status_labels["Buffer"].config(text=f"Buffer: {parts[8]}")
                self.status_labels["Search"].config(text=f"Search: {parts[10]}")
                self.status_labels["Lock"].config(text=f"Lock: {parts[12]} {parts[13]}")
...
#                     for sentence in data.splitlines():
